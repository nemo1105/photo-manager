...
- `command.command` is a Go `text/template` string. It renders at command start against one raw field, `.CurrentFile`, which carries the selected image's absolute path.
- Supported command-template helpers are `shell`, `powershell`, `sh`, `slash`, `urlquery`, `pssingle`, and `psdouble`. The product does not inject current-directory variables or a broader placeholder DSL.
...
- Config shape:
  ...
  - `command.command` stores the template string that is rendered before being sent to the platform shell. The template contract exposes `.CurrentFile` and the documented helper functions for quoting or embedding that path.
...
- `GET /api/slideshow`: current directory image list plus localized action labels and action availability.
  ...
  - `POST /api/command/start`: validates a `command` action against the current slideshow state, renders the configured command template with `.CurrentFile`, reserves one interactive terminal session, and returns the final runtime command text plus the alias-based terminal title.
...
- `command.command` intentionally stays as one template-backed shell string with only one data field, `.CurrentFile`; the app does not provide a broader placeholder DSL or auto-injected current-directory variables.
