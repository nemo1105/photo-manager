...
- [x] Refreshing or directly loading browser mode while a session is active ends that session instead of reopening browser mode with an active-session state.
...
- Browser:
  ...
  - `s` opens settings.
  - Loading browser mode ends any active session; browser mode does not expose a separate end-session key.
- Preview:
